...
const emptyActivity = { projectId: '', sector: 'agriculture', categoryChoice: '', category: '', activity: '', description: '', quantity: '', costUsd: '', signed: false, approved: false, status: 'Pending' };
...
function App() {
  ...
  const loadData = async () => {
    ...
    try {
      const results = await Promise.allSettled([
        ...
        fetchJson('/api/approvals'), fetchJson('/api/managers')
      ]);
      const [summaryResult, projectResult, activityResult, approvalResult, managerResult] = results;
  ...
  const chooseProject = (projectId) => {
    ...
    setActivityForm((current) => {
      ...
      // The preset category list is sector specific, so a sector change clears it.
      const keepCategory = sector === current.sector;
      return { ...current, projectId, sector, categoryChoice: keepCategory ? current.categoryChoice : '', category: keepCategory ? current.category : '' };
    });
  };
